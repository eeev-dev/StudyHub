...
import com.example.studyhub.viewmodels.plans.UpdateViewModel
...
fun UpdatePlanScreen(
    ...
    if (plan != null) {
        ...
        var content by rememberSaveable { mutableStateOf(plan.content) }
        var deadline by rememberSaveable { mutableStateOf(plan.deadline) }
        ...
        Column(
            ...
            Row(
                ...
                Button(
                    onClick = {
                        if (title == "" || content == "" || deadline == null) Toast.makeText(
                            context,
                            "Нужно заполнить все поля",
                            Toast.LENGTH_SHORT
                        ).show()
                        else {
                            viewModel.updatePlan(plan, title, content, deadline!!)
                            navController.navigate("plans_screen") {
                                popUpTo(navController.currentDestination?.id ?: 0) {
                                    inclusive = true
                                }
