...
class SettingsViewModel @Inject constructor(
    ...
    fun getEnabled() {
        viewModelScope.launch {
            _notificationsEnabled.value = dataStoreManager.getIsRemind()
        }
    }

    fun setNotificationsEnabled(enabled: Boolean) {
        viewModelScope.launch {
            dataStoreManager.saveIsRemind(enabled)
    ...
    fun cancelNotifications() {
        viewModelScope.launch {
            reminders.collect { itemList ->
                itemList.forEach { reminder ->
                    println("Удаление напоминаний")
                    val pendingIntent = Utils.getPendingContent(
                        appContext,
                        reminder.id.toInt(),
                        reminder.text,
                        reminder.url
    ...
    fun scheduleNotification(
        ...
        viewModelScope.launch {
            ...
            for (weekOffset in 0 until 26) {
                val triggerTime = baseTriggerTime + weekOffset * 7L * 24 * 60 * 60 * 1000 // смещение на неделю в миллисекундах
